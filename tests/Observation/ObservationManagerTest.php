 ...
 */
class ObservationManagerTest extends BaseCase
{
    ...
    public static function setupBeforeClass($fixtures = '12_Observation/manager'): void
    {
    ...
    public function testFilteredEventJournalUuid()
    {
        ...
        $node->setProperty('ref', $node, PropertyType::WEAKREFERENCE);
        $session->save();
        $this->produceEvents($session);
    ...
    public function testFilteredEventJournalNodeType()
    {
        ...
        $mixin = reset($mixins);
        $this->assertInstanceOf(NodeTypeInterface::class, $mixin);
    ...
    {
        $filter = $observationManager->createEventFilter();
        $filter->setNodeTypes(['non:existing']);
     ...
     * @param array                 $events  An array of type [[eventType, eventPath]] containing all the expected events in an arbitrary order.
     */
    ...
    protected function expectEvents(EventJournalInterface $journal, $startDate)
    {
        ...
        $events = $this->expectEventsInAnyOrder(
            ...
            [
                ...
                [EventInterface::NODE_MOVED, $this->nodePath.'/moved'],
            ]
        );
        foreach ($events as $event) {
